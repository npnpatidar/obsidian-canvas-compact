/**
 * Verification harness for the dagcola layout engine.
 *
 * Runs synthetic canvases through dagcolaLayout() and checks the hard guarantees.
 */
import {
  dagcolaLayout,
} from "../src/dagcola";
...
function run(
  ...
  strict = true,
  useCola = false
): void { // webcola is experimental, disabled by default
  const dir = direction as (typeof DEFAULT_CLEAN_OPTIONS)["direction"];
  const { nodes: outNodes, edges: outEdges, report } = dagcolaLayout(
    nodes as never[],
    edges as never[],
    { ...DEFAULT_CLEAN_OPTIONS, direction: dir, useColaRefinement: useCola }
  );
  ...
  console.log(`\n${name} [${direction}]${useCola ? " +cola" : " -cola"}`);
...
// Test without cola refinement
run("Mind-map tree (no cola)", tree, treeEdges, "top-to-bottom", undefined, true, false);
run("4-cycle (no cola)", cycle, cycleEdges, "top-to-bottom", undefined, true, false);
